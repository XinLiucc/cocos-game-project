...
# 阈值效果：某项硬技能练到门槛才解锁，接该项对应车型时在 repair_time_multiplier
# 基础上再打折，且不受该函数 [0.7x, 1.3x] 钳制。
# 2026-09-04：门槛数值重新校准。三维下软上限均分是 150/3=50，原 35 已低于均值，不刻意
# 专精也能顺手达到，门槛名存实亡。改成 60——按原 35 相对老均值 30 的比例（117%）换算到
# 新均值 50 约为 58，取整数 60，需要明显偏科（少练另外两项）才能摸到
const SKILL_ATTRIBUTE_THRESHOLD := 60
